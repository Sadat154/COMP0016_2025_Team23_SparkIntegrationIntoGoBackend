...
class Command(BaseCommand):
    help = "Create and populate Elasticsearch indices for SPARK (framework agreements and warehouse stocks)"
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--only-warehouse-stocks",
            action="store_true",
            default=False,
            help="Only process warehouse stocks index",
        )
        parser.add_argument(
            "--batch-size",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        Order of operations:
            ...
            2. Create warehouse stocks index (if not skipped)
            3. Bulk index framework agreements (if not skipped)
            4. Bulk index warehouse stocks (if not skipped)
        """
        ...
        only_ws = options.get("only_warehouse_stocks", False)
        batch_size = options.get("batch_size", 500)

        process_fa = not only_ws
        process_ws = not only_fa
        ...
        if not skip_create:
            ...
            if process_ws:
                logger.info("Creating warehouse stocks index...")
                try:
                    call_command("create_warehouse_index")
                    logger.info("Warehouse stocks index created successfully.")
                except Exception as ex:
                    logger.error(f"Failed to create warehouse stocks index: {ex}")
                    raise
        ...
        if not skip_bulk:
            ...
            if process_ws:
                logger.info("Bulk indexing warehouse stocks...")
                try:
                    call_command("bulk_index_warehouse_stocks", batch_size=batch_size)
                    logger.info("Warehouse stocks bulk indexing completed.")
                except Exception as ex:
                    logger.error(f"Failed to bulk index warehouse stocks: {ex}")
                    raise
